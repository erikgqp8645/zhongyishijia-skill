...
19. `references/install_workflow.md` for the GitHub release install workflow — 3 releases total (v1.1-lfs-data: evidence_cards.jsonl 269MB + books_json.tar.gz 75MB; v1.2-zysj-db: zysj.db 711MB / SHA256 `2ea61834...`, 2026-08-24 发布; v1.0-raw-sqlite: 原始 660MB 历史版本), which to download when, and the 3-data-layer architecture (jsonl + books_json/ + zysj.db).
19a. `references/sqlite_deployment_recipe.md` for the **end-to-end SQLite 部署验证 SOP** (2026-08-17 实测固化 + 2026-08-24 release v1.2-zysj-db 发布) — 从 GitHub release `v1.2-zysj-db` 下载 `zysj.db` (711MB, SHA256 `2ea61834...`) → 校验 SHA256 + 4 表行数 + GBK 编码 → 替换 `<repo>/references/external/zysj.db`（仓库内 SQLite 永远叫这个名字，0 字节占位符替换为 711MB 完整版）→ 跑 `find_sqlite_path()` 四级查找确认 → 跑 `python3 scripts/symptom_query.py "痰癖"` 烟雾测试必须命中 ≥1 张方剂。**Erik 偏好：保持 `zysj.db` 文件名不变**（所有现有脚本/文档引用此名，迁移时改脚本而非改名）。
...
27. `references/wenyao_query_workflow.md` for 「方剂专题查询工作流」(2026-08-17 新增) — **3 阶段 4 步流程**：用户语言→中医术语（关键词扩展）→ SQL 全库精筛（70,350→115）→ 分类与文档化（按 10 大功效 + 9 朝代溯源 + 30 核心药 + 3 实战配方）。**适用任何「X 主题方剂查询」**（明目方/安胎方/外科方/喉科方/妇科方/儿科方）。实战案例：闻药大全 115 张方剂。5 大陷阱（编码一刀切/关键词不够宽/不含真动词/数据库归类特殊/重复方剂）。触发词：「方剂专题查询」「查 X 方剂」「X 主题方剂」「全库方剂」「明目方」「安胎方」「外科方」「喉科方」「妇科方」「儿科方」。
28. `references/zhongyi_source_citation_principle.md` for 「中医方剂引用原文 5 大铁律」(2026-08-17 新增) — **所有 references/*.md 专题文档的引用原则**:① 原文**不带省略号**(Erik 硬性偏好·2026-08-17);② SQLite 双编码修复(**zysjyj=GBK / zysjllsj=UTF-8 不能一刀切**);③ **不删减**讲师/原文内容(项目愿景「不编造」原则);④ **可溯源**(每条引用必有 chunk_id);⑤ **朝代溯源**按可查证据分级(A/B/C/D 级)。整合 tanpi v3.2 + wenyao 大全实战。触发词:「中医方剂」「引用原文」「省略号」「溯源」「朝代」「编码」「不编造」「不删减」。
29. `references/kouqiang_kuiyang_zhenjiu.md` for 「口腔溃疡针灸方剂」专题(2026-08-27 新增) — 30 首方剂库方剂 + 35 条古籍理论原文,横跨东汉《金匮》→ 隋《病源》→ 唐《千金》→ 宋《圣惠/圣济》→ 金《保命》→ 元《丹溪》→ 明《肘后歌/外科正宗/类经图翼/本草纲目》→ 清《医学读书记/四圣心源》→ 现代医案。**核心创新**:① **5 大流派分类**(穴位贴敷 / 局部敷贴 / 远近端针刺 / 涌泉灸 / 联合治疗);② **20 个核心穴位主治详解**(劳宫/合谷/承浆/地仓/颊车/涌泉/照海/太溪等);③ **4 大穴位贴敷法**(涌泉/脚心/手足心/局部敷贴);④ **朝代演变轴**;⑤ **辨证分型·针灸取穴速查表**(10 证型);⑥ **3 首临床实战首推方**(吴茱萸+附子涌泉贴敷 / 草乌+南星+生姜手足心 / 合谷+地仓+黄连犀角汤)。**关键临床证据**:吴茱萸+附子敷涌泉治鹅口疮 170 例有效率 98%;三颗针治口腔炎 34 例全部治愈。触发词:「口腔溃疡针灸」「口疮针灸」「鹅口疮灸法」「狐惑针灸」「白塞氏针灸」「口糜针灸」「涌泉穴贴敷」「穴位贴敷口疮」「复发性口疮」。
30. `references/modern_classical_tcm_term_expansion.md` for 「现代病名↔古籍病名 扩展手册」(2026-08-27 新增) — **口腔溃疡专题蒸馏实战固化的类级技巧**:中医世家 2012-2014 数据库几乎不含现代医学病名,扩展为古籍术语后才命中(扩展前 5 张 vs 扩展后 250+ 张)。**3 步 SOP**:① 识别现代病名 ② 查扩展词典(7 大类:口腔/皮肤/消化/呼吸/心脑/妇科/五官) ③ 多 OR 模式 + 双源 SQL 取证。**核心洞察**:**口腔溃疡的针灸外治核心是穴位贴敷(60%+ 方剂)而非传统针刺**——吴茱萸/附子/天南星/草乌醋调贴涌泉/脚心/手足心,引火下行。触发词:「现代病名扩展」「古籍病名」「口腔溃疡↔口疮」「白塞氏↔狐惑」「湿疹↔湿疮」「胃炎↔胃脘痛」「穴位贴敷口疮」「引火下行」。
31. `references/gaoyao_jingjianyaotuiteng.md` for 「颈肩腰腿痛膏药方大全」(2026-09-07 新增) — **121 张膏药方剂全库溯源**(清《理瀹骈文》吴尚先「薄贴论」+「筋骨闪挫膏药方」+「风湿诸般疼痛膏药」+「贴腰膏」核心,横跨唐《千金要方/外台秘要》→ 宋《太平圣惠方/圣济总录》→ 明《外科正宗/证治准绳/医学纲目》→ 清《医宗金鉴/张氏医通/外科证治全生集/急救广生集》→ 现代《药典》)。**核心创新**:① **8 大功效分类**(跌打损伤 34/腰痛 27/风寒湿痹 22/全身筋骨痹痛 15/筋脉麻木拘挛 13/腿膝足痛 5/颈项痛 3/半身不遂瘫痪 2);② **Top 40 高频核心药本草溯源**(当归 102/附子 56/肉桂 42/麝香 40/细辛 34/没药 33/乳香 31);③ **5 大经典方根家族**(太乙膏/万应膏/狗皮膏/虎骨膏/阳和解凝膏);④ **5 大临床实战首推方**(蠲痛五汁膏/贴腰膏/集宝疗痹膏/阳和解凝膏/苍梧道士陈元膏 安全/救急/峻烈 3 级);⑤ **临床决策算法**(5 步:急性/慢性→虚实→部位→寒热→安全性检查)。**关键临床证据**:**「薄贴论」吴尚先原话**「其病既有定所,在于皮肤筋骨之间,可按而得者,用膏贴之,闭塞其气,使药性从毛孔而入其腠理,通经贯络,或提而出之,或攻而散之,较之服药尤有力,此至妙之法也」。**膏药透皮吸收的现代药理学证实**:冰片/薄荷脑/樟脑为透皮促进剂,川乌/草乌含乌头碱镇痛,乳香/没药含 Boswellic acid 抗炎。触发词:「颈肩腰腿痛」「颈肩腰腿痛膏药」「筋骨疼痛膏药」「风湿疼痛膏药」「跌打损伤膏药」「腰痛膏药」「腿膝足痛膏药」「痹证膏药」「贴腰膏」「狗皮膏」「虎骨膏」「筋骨闪挫膏药」「风湿诸般疼痛膏药」。
32. `references/gaoyao_query_workflow.md` for 「中医外科膏药方专题查询工作流」(2026-09-07 新增) — **膏药专题查询 SOP 姊妹文档**(`wenyao_query_workflow.md` 母工作流是闻药·鼻吸·鼻烟专题,本文档专攻膏药外治)。**3 大专属约束**(vs 母工作流):① **约束 A 皮肤痹痛 vs 筋骨痹痛去重**(核心陷阱:皮肤病膏药如川槿皮膏/马齿苋膏/曼荆实丸也会出现「痹」字,但治风癣/鹅掌风非筋骨痹痛——必须用部位白名单「颈/项/肩/臂/腰/腿/膝/足/脚/背/脊/筋骨/骨节/肢节/关节」二次过滤);② **约束 B 古今同名方剂家族化呈现**(狗皮膏 14+ 变方 / 太乙膏 4+ 变方 / 万应膏 9+ 变方 / 紫金膏 15+ 变方——保留全部变方但在文档中家族化分组,而非简单去重);③ **约束 C 现代中成药 vs 古籍原方 分层**(基质差异:古方麻油+黄丹 / 近代方+松香蜂蜡 / 现代方橡胶+凡士林+羊毛脂+氧化锌+汽油——朝代溯源章节必须显式标注)。**3 大 SQL 实战陷阱**:① Python `'''...'''` 文档字符串内嵌 SQL 单引号冲突导致 0 命中(实战 700+ → 0 条);② 复合 WHERE 子句被 f-string 切错乱;③ zysjyj=GBK / zysjllsj=UTF-8 编码一刀切乱码。**实战数据**:121 张膏药方剂 / 76KB / 524 行 / 8 大功效分类 / Top 40 高频核心药 / 5 大经典方根家族 / 5 大临床实战首推方。触发词:「中医外科膏药」「中医外科膏方」「中医外科薄贴」「中医外科贴药」「膏药专题查询」「膏药方查询」「颈肩腰腿痛膏药查询」「跌打损伤膏药查询」「风湿疼痛膏药查询」「中医外科膏药工作流」「膏药工作流」。
33. `references/yin_chuang_treatment.md` for 「阴疮」专题 (2026-09-17 新增) — **23 张阴疮方剂 + 30+ 张阴蚀方剂全库溯源**,横跨东汉《金匮要略》→ 隋《诸病源候论》→ 唐《千金要方》→ 宋《妇人良方/校注妇人良方》→ 明《外科正宗》→ 清《医宗金鉴/医学纲目/长沙药解/本经疏证/陈素庵妇科补解》→ 现代《赵炳南临床经验集》。**核心创新**:① **5 大病机分类**(湿热下注/虫蚀蕴毒/寒湿凝滞/气血两虚/肝郁脾虚);② **病机时间线**(仲景立论 → 隋巢元方虫蚀学说 → 宋陈自明七情郁火 → 明陈实功三等说 → 清黄元御木郁下热注 → 现代赵炳南三联方);③ **23 张阴疮方剂按主方归类**(仲景原方2+内服主方7+温阳托毒方3+外用膏散方8+简验方3);④ **30+ 张阴蚀外治方剂按功效分类**(仲景原方2+经典外洗方7+内服托毒方5+散剂膏剂8+简验方8);⑤ **Top 6 高频核心药本草溯源**(乳香/黄柏/黄耆/当归/白芷/甘草);⑥ **临床决策流程图**(症状→病机→方剂);⑦ **阴疮与狐惑(白塞氏病)鉴别**——仲景「蚀于喉为惑,蚀于阴为狐」,狼牙汤治阴、甘草泻心汤治喉。**关键临床证据**:**「阴蚀多发于嫠寡(寡妇),证必咳逆经闭,骨蒸寒热」(邹澍《本经疏证》)**;**「痘疹阴疮,则当以黄耆为君,人参为臣」**(黄耆本草)。触发词:「阴疮」「阴蚀」「阴中生疮」「妇人阴疮」「外阴溃疡」「前阴溃疡」「妇人隐疾」「下疳阴疮」「阴痒阴蚀」「产后阴蚀」「蠹疮」「白塞氏阴部表现」「糖尿病合并外阴感染」「嫠寡阴蚀」。
