...
- ✅ 已实现:Skill C「跨祖方家族对比」(`cross_family_compare.py`)— 输入 2-3 个方剂名,对比它们在哪几个祖方家族里出现
- ✅ 已实现:Skill C.2「病证反查」(`zheng_lookup.py`)— 输入病证关键词(如「寒湿腰痛」「痞证」「咳嗽」),列出该病证在《醫通祖方》中涉及的所有祖方/变法方
- 📋 待实现:Skill C.3 治法归类 — 33 个祖方按治法(汗/吐/下/温/补/清/消/和)分类
- 📋 待实现:Skill C.4 方剂族谱可视化(mermaid) — 把方族谱生成 mermaid 图
- 📋 待实现:Skill D「祖方 vs 类方书」横向对比 — 同一变法方在《医通祖方》/《证治准绳类方》/《退思集类方歌注》/《秘传证治要诀及类方》4 本书里的不同注解
