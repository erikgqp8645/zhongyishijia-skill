...
| 6 | **Python 三引号字符串 + SQLite 单引号 LIKE 双重转义陷阱（2026-09-07 颈肩腰腿痛膏药实战固化）** | 用 `'''..."""..."'''` 或 docstring 三引号包 SQL，**内层 `'LIKE '%痹%''` 会让 Python 解析器在 `'%痹%'` 第二个单引号提前闭合字符串**，导致 `clauses_a`/`clauses_b` 整个变成 `LIKE '%'` 残句，运行时静默返回 0 条（不是报错！）。**两类修复**：(a) **内层 LIKE 模式改用双引号**：`LIKE "%膏%"`（SQLite 标准支持），整个 SQL 字符串用单引号或无前缀三引号包；(b) **动态 f-string 拼接**：列表循环生成 `clauses.append(f\"ChuFang LIKE '%{{k}}%'\")`，避开单引号嵌套。**根因判定**：用 sqlite3 走同样的 SQL，0 条但 Python 报 `sqlite3.OperationalError` 或 `len(list(cur))==0` 而 `OR` 嵌套复杂 → 99% 是转义问题，跑简化版单条件 SQL（`SELECT COUNT(*) FROM zysjyj WHERE MingCheng LIKE '%膏%' LIMIT 5`）确认数据库有数据后再排查字符串转义 |
...
| **颈肩腰腿痛膏药** | 贴之 / 薄贴 / 摊贴 / 敷之 + 痹/历节/风湿/筋骨 | 121 | ✅ gaoyao_jingjianyaotuiteng.md |
...
### 7.1 膏药类专题的关键差异（2026-09-07 实战固化）

膏药方查询与「闻药」「明目方」类查询有 3 个关键差异，编码进 SOP：

**1. 制剂形态 ≠ 病证**。方剂库中「方名含『膏』」未必是治疗目标病证的膏药（很多是治疗眼/皮肤/妇科/儿科的膏剂）。**必须按主治导向**——只用「**病证关键词必须在 GongNengZZ（功能主治）命中**」+「**病证关键词必须在 ChuFang（处方）或 GongNengZZ 中明确出现部位词（颈/项/肩/臂/腰/腿/膝/足/脚/背/脊/骱/筋骨/骨节/肢节/关节）**」双重约束才能锁定。

**3 步走漏斗**：
-  Step A：方名含膏/薄贴/贴（粗筛 2526 条）
-  Step B：合并「处方/主治含薄贴/摊贴/贴之/敷之/贴患处/烘贴/熨之」外用动词（合并 2647 条去重）
-  Step C：精筛「GongNengZZ 必须含痹痛专属词（痹/历节/白虎风/鹤膝/痛风/风湿/寒湿/湿痹/半身不遂/筋骨/骨节/肢节/拘挛/腰腿/膝痛/颈项/项强…）」+「必须含至少一个痹痛部位」（121 条命中）

**2. 皮肤膏药 = 主治词陷阱**。`风癣/顽癣/鹅掌风/疥` 等皮肤病术语常带「顽痹/瘙痒/抓搔」字样，主治含「痹」字但与筋骨痹痛无关。**必须在第二步排除**：若主治含 `风癣/顽癣/鹅掌风/疥/湿癣`，且不含 `腰/腿/膝/肩/颈/项/臂/脊/背/筋骨` 部位词，直接剔除。

**3. 经典膏药方剂多在 `zysjllsj` 而非 `zysjyj`**。吴尚先《理瀹骈文》「薄贴论」「筋骨闪挫膏药方」「风湿诸般疼痛膏药」「贴腰膏」等清代膏药专题文献，**全部位于 zysjllsj 临床理论库**（zysjyj 方剂库 0 命中）。所以必须**双源取证**：
-  zysjyj：用方剂库 121 条精筛的现代/明清/民国方剂
-  zysjllsj：找含「薄贴/贴之/敷之/筋骨/腰/腿/膝/肩/颈/项/风湿/历节」的 BiaoTi 与 NeiRong，按朝代溯源
-  临床文献溯源路径：`SELECT ID, BiaoTi FROM zysjllsj WHERE BiaoTi IN ('薄贴论','第二节·薄贴各方','筋骨闪挫膏药方','风湿诸般疼痛膏药','贴腰膏','薄贴（膏药）','膏药类')` — 找到清代膏药专题文献全文

**4. 必做的烟雾测试**：10 张核心膏药方剂（狗皮膏/虎骨膏/蠲痛五汁膏/集宝疗痹膏/阳和解凝膏/苍梧道士陈元膏/筋骨闪挫膏药方/薄贴论/贴腰膏/摩腰膏）在 zysjyj 几乎 0 命中，**必须在 zysjllsj 二次查询**；不要把 zysjyj 0 命中误判为「数据缺失」。
...
### v1.1 (2026-09-07) — 颈肩腰腿痛膏药专题实战固化

- **新增实战案例** `references/gaoyao_jingjianyaotuiteng.md`（121 张 / 76KB），作为方剂专题查询工作流的**第二个 ✅ 实战案例**（与闻药大全并列）
- **§七扩展应用表**新增 1 行 ✅ 标记：颈肩腰腿痛膏药（121 张）
- **§七.1 新增「膏药类专题的关键差异」**：
  1. 制剂形态 ≠ 病证（方名含膏 ≠ 治疗目标病证的膏药）
  2. 皮肤膏药 = 主治词陷阱（风癣/顽痹字样需剔除）
  3. 经典膏药方剂多在 zysjllsj 而非 zysjyj（吴尚先《理瀹骈文》专题文献全在临床理论库）
  4. 必做双源取证 + 烟雾测试（10 张核心膏药方剂在 zysjyj 几乎 0 命中）
- **§六陷阱表新增第 6 条**：Python 三引号字符串 + SQLite 单引号 LIKE 双重转义陷阱（膏药专题实战反复掉进同一坑两次，固化进 SOP）
